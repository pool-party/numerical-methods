...
$$
{U \over \partial b_0} = {U \over \partial b_1} = 0,
$$

что эквивалентно

$$
{U \over \partial b_0} = 2 * \sum_{i = 1}^n (y_i - b_0 + b_1 * x_i) *(-1) \\
{U \over \partial b_1} = 2 * \sum{i = 1}^n (y_i - b_0 + b_1 * x_i) *(-x_i),
$$
...
$$
y = a_0 + a_1 * x + a_2 * x_2 + \dots + a_m * x_m
$$
